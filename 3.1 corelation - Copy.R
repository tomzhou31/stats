...
fig_il6 <- ggplot()+
  ...
  annotate("text", x = 1.4, y = 285, size=7, label = "paste(italic(p) , \" = 0.040\")", parse = TRUE)  +
  annotate("text", x = 3.7, y = 285, size=7, label = "paste(italic(p) , \" < 0.0001\")", parse = TRUE)  +
...
fig_il8 <- ggplot()+
  ...
  annotate("text", x = 1.4, y = 285, size=7, label="italic(p) == 0.033", parse=TRUE) +
  annotate("text", x = 3.7, y = 285, size=7, label="italic(p) == 0.00017", parse=TRUE) +
...
fig_pce_il2 <- ggplot()+
  ...
  # annotate("text", label = p_il8, x = 1.5, y = 100, size=6) +
  annotate("text", x = 0.8, y = 2800, size=7, label="italic(p) == 0.0091", parse=TRUE) +
  geom_text(x = 14.25, # Set the position of the text to always be at '14.25'
            hjust = 0,
            size = 8) +
  theme_bw(base_size = 25) +
...
# tiff("output/FIG5_corpus_il2.tiff" , units="in", width=6, height=3.5, res=300)
# il2
# dev.off()
...
annotate("text", x = 380, y = 1.15, size=7, label = "paste(italic(R) , \"= -0.65, \", italic(p) , \" = 0.030\")", parse = TRUE)
